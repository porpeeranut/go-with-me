...
<button id="btn_add" onclick="window.document.location='main.php?option=add&TYPE=location';" class="btn btn-default">Add</button>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#btn_previous").click(function(){
            if (start > 0)
                start -= n;
            $.get("module/administrator/get.php?option=location&s="+start+"&n="+n, function(result) {
                var obj = jQuery.parseJSON(result);
                showData(obj);
            });
        });
        ...
        function showData(obj) {
            ...
            for (i=0;i<obj.data.length;i++) {
                ...
                row += '<a href="#">';
                row += '<img onclick="window.document.location=\'main.php?option=badge_type_detail&TYPE=locations&ID='+ID+'&NAME='+NAME+'&DETAIL='+DETAIL+'\';" class="img-responsive" src="images/locations/'+obj.data[i].ID+'.jpg" alt="">';
                row += '</a>';
                row += '<h3>';
                row += obj.data[i].ID;
                row += '</h3>';
        ...
        $("#btnSearch").click(function(){
            $.get("module/administrator/search.php?option=location&keyword="+$("#searchTxt").val()+"&attr="+$("#searchType").val()+"&s=0&n=8", function(result) {
                var obj = jQuery.parseJSON(result);
                row = "";
                for (i=0;i<obj.data.length;i++) {
                    ID = obj.data[i].ID;
